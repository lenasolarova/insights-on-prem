...
# Auto-discover the latest 2.16 snapshot — falls back to a known-good tag if Quay is unreachable.
CONSOLE_IMAGE=$(curl -s "https://quay.io/api/v1/repository/stolostron/console?includeTags=true" 2>/dev/null |   python3 -c "import sys,json; tags=sorted([t for t in json.load(sys.stdin)['tags'] if '2.16.0-SNAPSHOT' in t]); print('quay.io/stolostron/console:'+tags[-1])" 2>/dev/null ||   echo "quay.io/stolostron/console:2.16.0-SNAPSHOT-2026-04-06-05-22-18")
